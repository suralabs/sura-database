...
class Helpers
{
	...
	public static function dumpSql(string $sql, array $params = null, Connection $connection = null): string
	{
		...
		$sql = preg_replace_callback('#\?#', function () use ($params, $connection): string {
			...
			if (
				...
				$length = Sura\Utils\Strings::length($param);
				$truncated = Sura\Utils\Strings::truncate($param, self::$maxLength);
	...
	public static function loadFromFile(Connection $connection, string $file, callable $onProgress = null): int
	{
		...
		if (!$handle) {
			throw new Sura\FileNotFoundException("Cannot open file '$file'.");
		}
	...
	): Sura\Bridges\DatabaseTracy\ConnectionPanel {
		$panel = new Sura\Bridges\DatabaseTracy\ConnectionPanel($connection, $blueScreen);
